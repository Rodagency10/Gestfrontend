...
const useSales = () => {
  ...
  const getSalesByCashier = useCallback(async (cashierId: string) => {
    setLoading(true);
    setError(null);

    try {
      const data: SalesResponse = await makeRequest(`/admin/sales/cashier/${cashierId}`);
      setSales(data.sales || []);
      return data.sales;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Erreur lors du chargement des ventes';
      setError(errorMessage);
      console.error('Erreur lors du chargement des ventes par caissier:', err);
      return [];
    } finally {
      setLoading(false);
    }
  ...
  const calculateSalesStats = useCallback((salesData: Sale[] = sales) => {
    ...
    const salesByCashier = salesData.reduce((acc, sale) => {
      acc[sale.username] = (acc[sale.username] || 0) + Number(sale.total_amount);
      return acc;
    }, {} as Record<string, number>);
  ...
  const filterSales = useCallback((filters: {
    ...
    return sales.filter(sale => {
      ...
      if (filters.dateStart && saleDate < new Date(filters.dateStart)) return false;
      if (filters.dateEnd && saleDate > new Date(filters.dateEnd)) return false;
